...
for i in range(3):
	...
	while True:
		...
		try:
			...
			if temphum !=[]:
				#print(temphum)
				TimeTemperatureHUmidity.append([st,float(temphum[0]),float(temphum[1])])
				jj = jj+1
				print(jj,st,float(temphum[0]),float(temphum[1]))
				if jj>=threshold:
					break
	...
	ts = time.time()
	st = datetime.datetime.fromtimestamp(ts).strftime('%Y_%m-%d_%H_%M_%S')
	df = pd.DataFrame(data = TimeTemperatureHUmidity, columns=["Time","Temperature", "Humidity"])
